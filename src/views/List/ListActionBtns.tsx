...
const ListActionBtns = ({
      ...
      switch (actionType) {
        case 'set_watched':
          response = await handleWatchedMovieRequest(
            ...
            isMovieOrSerie,
            true,
          ); // Ajout dans la liste des films vus (à noter)
        ...
        case 'set_unwatched':
          ...
          response = await handleWatchedMovieRequest(
            ...
            isMovieOrSerie,
            false,
          ); // Suppression de la liste des films vus
          handleOpenSnackbar(`${response.message} ${movieTitle}.`);
          break;
        case 'note':
  ...
  return (
    <Stack direction="row" spacing={2}>
      {listSectionIndex === 0 ? (
        <>
          <ListButton btn={listButtons[0]} handleBtnAction={handleBtnAction} />
          <ListButton btn={listButtons[2]} handleBtnAction={handleBtnAction} />
          <ListButton btn={listButtons[3]} handleBtnAction={handleBtnAction} />
        </>
      ) : (
        <>
          <ListButton btn={listButtons[1]} handleBtnAction={handleBtnAction} />
          <ListButton btn={listButtons[2]} handleBtnAction={handleBtnAction} />
          <ListButton btn={listButtons[3]} handleBtnAction={handleBtnAction} />
        </>
      )}
