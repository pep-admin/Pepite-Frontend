...
const PreferencesComponent = () => {
  ...
  const submitGenres = async() => {
    if(favoriteGenres.length < 3 || favoriteGenres.length > 6) {
      setShowAlert({ display: true, error: false, severity: 'info', message: 'Vous devez choisir entre 3 et 6 genres préférés.' });
    } else {
      ...
      try {
        ...
        setShowAlert({ display: true, error: true, severity: 'error', message: 'Erreur serveur : impossible de mettre à jour les genres préférés.' });
      }
    }
  }
      ...
      <Grid container spacing={2}>
        {
          shortGenresMovieList.map((genre) => {
            return(
              <PreferencesCard key={genre.id} genre={genre} favoriteGenres={favoriteGenres} setFavoriteGenres={setFavoriteGenres} />
            )
          })
        }
      </Grid>
