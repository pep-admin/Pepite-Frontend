...
import React, { MutableRefObject, FC, useState, Dispatch, SetStateAction } from 'react';
...
const SwipeCard3: FC<SwipeCard3Props> = React.memo(({ 
  ...
  const opacityValue = useTransform(motionValue, [-200, -150, 0, 150, 200], [0, 0.8, 1, 0.8, 0]); // 0 d'opacité sur 200px sur l'axe des x
    ...
    <motion.div
      ...
      onDragEnd={(_, info) => {
        ...
        if (Math.abs(info.offset.x) > swipeThreshold) {
          ...
          animate(motionValue, 0, { type: 'spring', stiffness: 150, damping: 25 });
          animate(rotateValue, 0, { type: 'spring', stiffness: 150, damping: 25 });
          animate(opacityValue, 1, { type: 'spring', stiffness: 150, damping: 25 });
        }
      }}
