...
const RatingPublish = ({
  ...
  const generateMessage = (error: boolean) => {
    if (error) {
      ...
      if (isCriticOrAdvice === 'advice' && !friendSelected) {
        return `Vous devez choisir un(e) ami(e) à qui conseiller ${
  ...
  const handleConfirmation = (
    ...
    showAlert({
      ...
      severity: `${alertType}`,
      onConfirm: () =>
        !criticAdviceId // Pas de critique déjà existante ?
    ...
    try {
      if (isCriticOrAdvice === 'critic') {
        ...
        if (advice.exists) {
          ...
          handleOpenSnackbar(
            `Vous avez conseillé ${movieTitle} à ${friendFullname}.`,
          );
          setMovieSelected(null);
        }
      }
  ...
  const updateCriticAdvice = async (criticAdviceId: number) => {
    try {
      ...
      handleOpenSnackbar(
        'Erreur: impossible de modifier la critique existante.',
      );
    }
  };

  return (
    <Stack spacing={6} padding="30px 0 40px 0">
      <Stack>
        <Typography
          component="h2"
          color="text.primary"
        ...
        >
          {ratingSectionIndex === 0 ? 'PUBLICATION' : 'ENVOI DU CONSEIL'}
        </Typography>
        <Typography
          component="h2"
          color="gray"
          ...
          marginTop="4px"
        >
          {ratingSectionIndex === 0 ? 'Publique ou privée' : 'Privé'}
        </Typography>
      </Stack>
      <Stack
        direction="row"
          ...
          <Stack spacing={1}>
            <Button
              sx={{
                ...
                color: '#f1f1f1',
                outline: `1px solid ${
                  publicationChoice === 'publique' ? '#E7AE1A' : '#2D2D2D'
                }`,
                fontSize: '0.75em',
                fontWeight: '400',
              }}
          ...
          <Stack spacing={1}>
            <Button
              sx={{
                ...
                color: '#f1f1f1',
                outline: `1px solid ${
                  publicationChoice === 'privée' ? '#E7AE1A' : '#2D2D2D'
                }`,
                fontSize: '0.75em',
                fontWeight: '400',
              }}
            ...
            >
              {ratingSectionIndex === 0 ? 'PRIVÉE' : 'PRIVÉ'}
            </Button>
            <Typography
              align="center"
              fontWeight="200"
      ...
      <Stack alignItems="center">
        <Button
          ...
          sx={{
            ...
            padding: '7px 12px 5px 12px',
            color: '#011212',
            backgroundColor: checkIfPublishValid()
              ? '#E7AE1A !important'
              : '#383838',
          }}
          onClick={() =>
        ...
        >
          {ratingSectionIndex === 0 ? 'PUBLIER' : 'CONSEILLER'}
        </Button>
      </Stack>
    </Stack>
  );
