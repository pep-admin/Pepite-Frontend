...
const SwipeComponent3 = ({
  ...
  const [isUnwanted, setIsUnwanted] = useState(movies[currentIndex]?.is_unwanted);
  ...
  const handleSnackbarMessage = (btnChoice: string) => {
    ...
    switch (btnChoice) {
      case 'unwanted':
        if (!isUnwanted) {
          handleOpenSnackbar(`"${movies[currentIndex].title}" ne vous sera plus proposé`);
        } else {
          handleOpenSnackbar(`"${movies[currentIndex].title}" pourra vous être proposé`);
        }
        break;
      case 'wanted':
        if (!isWanted) {
          handleOpenSnackbar(`"${movies[currentIndex].title}" a été ajouté à votre liste`);
        } else {
          handleOpenSnackbar(`"${movies[currentIndex].title}" a été retiré de votre liste`);
        }
        break;
      case 'watched':
        if (!isWatched) {
          handleOpenSnackbar(`Vous avez déjà vu "${movies[currentIndex].title}"`);
        } else {
          handleOpenSnackbar(`Vous n'avez pas vu "${movies[currentIndex].title}"`);
        }
        break;
      default:
        break;
  ...
  const handleActions = async (btnChoice: string) => {
    ...
    switch (btnChoice) {
      case 'unwanted':
        response = await handleUnwantedMovieRequest(movie.id, movieOrSerie, !isUnwanted);
        break;
      case 'wanted':
        response = await handleWantedMovieRequest(movie.id, movieOrSerie, !isWanted);
        break;
      case 'watched':
        response = await handleWatchedMovieRequest(movie.id, movieOrSerie, !isWatched);
        break;
      default:
        break;
    }
  ...
  return (
    <>
      ...
      >
        { movies.length > 0 &&
          <Stack position="absolute" top="75px" right="6%" zIndex="1000">
            <Badge
              badgeContent={ countActiveFiltersSwipe(typeChosen, countryChosen, genreChosen, ratingChosen, periodChosen) }
              showZero
              overlap="circular"
              sx={{
                '& .MuiBadge-badge': {
            ...
            >
              <CustomButton btntype={'filter'} onClick={() => setAreFiltersOpened(!areFiltersOpened)}>
      ...
      />
      {error.state ?
        <CustomAlert alertType='error' message={error.message} setOnAlert={setError} />
        :
        null
      }
    </>
