...
const NotificationsMenu = ({ page, anchorElNotif, openNotif, handleCloseNotifMenu, notifications}) => {
  ...
  const generateContent = (notificationType, firstName, lastName) => {
    switch(notificationType) {
      case 'friendRequest':
        return (<span><strong>{firstName} {lastName}</strong> souhaite vous ajouter à sa liste d'amis.</span>);
      case 'acceptedFriendRequest':
        return (<span><strong>{firstName} {lastName}</strong> a accepté votre demande d'amitié.</span>);
      case 'movieAdvice':
        return (<span><strong>{firstName} {lastName}</strong> vous a conseillé un nouveau film.</span>);
      case 'serieAdvice':
        return (<span><strong>{firstName} {lastName}</strong> vous a conseillé une nouvelle série.</span>);
      default:
        return null;
    }
  }
                ...
                <Stack direction='row' spacing={2}>
                  ...
                  <Stack>
                    <Typography variant='body2' maxWidth='200px'>
                      {generateContent(notif.type, notif.first_name, notif.last_name)}
                    </Typography>
                  </Stack>
                </Stack>
                { index !== notifications.length - 1 && <Divider sx={{ width: '100%', margin: '8px 0' }} /> }
              </MenuItem>
            );
          })
        }
